...
-- Add ForeignKey for contract_id on vehicles now that contracts exists
...
-- Mileage Readings (Leituras de Hodômetro)
...
-- Demobilizations (Desmobilização de Veículos)
...
-- 4. Enable Row Level Security (RLS) Policies for Security
...
-- Permissive policies allowing public select & insert/update for anon/authenticated (Configurable for Auth)
...
CREATE POLICY "Allow public read access to carriers" ON public.carriers FOR SELECT USING (true);
...
CREATE POLICY "Allow public read access to fines" ON public.fines FOR SELECT USING (true);
CREATE POLICY "Allow public all access to fines" ON public.fines FOR ALL USING (true);

CREATE POLICY "Allow public read access to mileage_readings" ON public.mileage_readings FOR SELECT USING (true);
...
CREATE POLICY "Allow public read access to demobilizations" ON public.demobilizations FOR SELECT USING (true);
